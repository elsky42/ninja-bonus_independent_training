...
func int B_TeachAttributePoints (var C_NPC slf, var C_NPC oth, var int attrib, var int points, var int teacherMAX)
{
	...
	if 		(attrib == ATR_STRENGTH)	{	realAttribute = oth.aivar[REAL_STRENGTH];	}	// Umwandeln von const-Parameter in VAR f�r folgende If-Abfrage
	else if (attrib == ATR_DEXTERITY)	{	realAttribute = oth.aivar[REAL_DEXTERITY];	}
	else if (attrib == ATR_MANA_MAX)	{	realAttribute = oth.aivar[REAL_MANA_MAX];	};
